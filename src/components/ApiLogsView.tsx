...
export function ApiLogsView() {
  ...
  return (
    <div>
      ...
      <div
        ref={workspaceRef}
        className="relative grid min-h-[390px] overflow-hidden rounded-lg border bg-card shadow-sm lg:grid-cols-[var(--log-pane-width)_minmax(0,1fr)]"
        style={{ '--log-pane-width': `${paneWidth}%` } as CSSProperties}
      >
        <div className="min-h-0 overflow-auto border-b lg:border-b-0 lg:border-r">
        ...
        <aside aria-label="Selected API log response" className="flex min-h-[280px] min-w-0 flex-col">
          {selected ? (
            <>
              ...
              <pre className="min-h-0 flex-1 overflow-auto bg-muted/30 p-3 text-xs leading-relaxed">{formatResponsePayload(selected.responseBody)}</pre>
            </>
